    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for location in locations:
                ...
                location_info = await location_element.inner_text()
                location_url_info = await location_element.query_selector('a').get_attribute('href') if location_element and await location_element.query_selector('a') else ''
                description = await location.query_selector('div.location-description').inner_text()
                category = "Food and Beverage"
                contact_element = await location.query_selector('div.location-contact')
                contact_info = await contact_element.inner_text().strip() if contact_element else ''
                hours_element = await location.query_selector('div.location-hours')
                hours_info = await hours_element.inner_text().strip() if hours_element else ''
