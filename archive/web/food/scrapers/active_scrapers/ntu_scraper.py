...
async def scrape_ntu(base_url):
    ...
    while True:
        ...
        listings = soup.select(
            "div.col-sm-8.col-md-9 li.search__results-item.col-sm-6.col-md-4"
        )
        if not listings:
            errors.append("No more listings found.")
            break
        for listing in listings:
            name = listing.select_one("div.img-card__body h3.img-card__title").get_text(
                strip=True
            )
            raw_location = (
                listing.select_one(
                    "span.img-card__label.location-label span.location"
                ).get_text(strip=True)
                if listing.select_one("span.img-card__label.location-label")
                else ""
            )
            clean_location = clean_string(raw_location)
            raw_description = (
                listing.select_one("p.img-card__info").get_text(strip=True)
                if listing.select_one("p.img-card__info")
                else ""
            )
