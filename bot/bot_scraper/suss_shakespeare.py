...
async def scrape_suss_dining(url):
    """
    Scrapes the SUSS site asynchronously.
    """
    scraped_data = []
    errors = []
        ...
        try:
            ...
            while True:
                ...
                for item in post_items:
                    ...
                    item_url = await item.query_selector(
                        "div.post-item-image.img-container-250 a"
                    )
                    url = await item_url.get_attribute("href") if item_url else ""
...
async def run_scraper(target_url):
    ...
    """
    details_list, errors = await scrape_suss_dining(target_url)
    if errors:
