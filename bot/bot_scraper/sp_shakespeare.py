...
async def scrape_sp_dining(url):
    """
    Scrapes the SP website for food and beverage details asynchronously.
    """
    scraped_data = []
    errors = []
    async with async_playwright() as p:
        browser = await p.chromium.launch(
            headless=False
        )  # Headless is False to allow visibility for debugging
...
async def run_scraper(target_url):
    ...
    """
    details_list, errors = await scrape_sp_dining(target_url)
    if errors:
