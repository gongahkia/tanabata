...
async def scrape_np_dining(url):
    """
    Scrapes the NP dining site asynchronously
    """
    scraped_data = []
    errors = []
    async with async_playwright() as p:
        # Launching the browser in non-headless mode to see scraping actions
...
async def run_scraper(target_url):
    ...
    """
    details_list, errors = await scrape_np_dining(target_url)
    if errors:
