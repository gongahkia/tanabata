...
def scrape_marina_bay_sands_restaurants(base_url):
    ...
    with sync_playwright() as p:
        ...
        try:
            page.goto(base_url)
            page.wait_for_selector("div.restauranttitlefilter__container--carddetails__card")
            print(f"successfully retrieved page URL: {base_url}")
            restaurant_cards = page.query_selector_all('div.restauranttitlefilter__container--carddetails__card')
            for card in restaurant_cards:
                name_element = card.query_selector('div.restauranttitlefilter__container--carddetails__card--contentarea div.title div.cmp-title a.cmp-title_link h3.cmp-title__text')
                url_element = card.query_selector('div.restauranttitlefilter__container--carddetails__card--contentarea div.title div.cmp-title a.cmp-title_link')
                location_element = card.query_selector('div.restauranttitlefilter__container--carddetails__card--details div.location-details div.location-details--address div.information')
                name = clean_string(name_element.inner_text()) 
                url = f"https://www.marinabaysands.com{url_element.get_attribute('href')}"
