    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            while True:
                ...
                next_page_array = [
                    page_el
                    for page_el in page.query_selector_all(
                        "div.nav-links a.page-numbers"
                    )
                    if page_el.inner_text().isnumeric()
                    and page_el.inner_text().strip() not in travelled_pages
                ]
                if next_page_array:
                    next_page = next_page_array[0]
                    print(
                        f"page numbers remaining: {[el.inner_text() for el in next_page_array]}"
                    )
                if len(next_page_array) == 0:
                    print("No more pages to navigate.")
                    break
                print(
                    f"current page: {current_page.inner_text()}, navigating to next page {next_page.inner_text()}..."
                )
                next_page.click()
