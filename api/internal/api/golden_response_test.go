...
func TestEntityGraphGoldenResponse(t *testing.T) {
	server, store := seededLineageServer(t)
	defer store.Close()

	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodGet, "/v1/graph/tanabata:frank-ocean?depth=2", nil)
	server.Router().ServeHTTP(recorder, request)
	if recorder.Code != http.StatusOK {
		t.Fatalf("graph status = %d, want 200 body=%s", recorder.Code, recorder.Body.String())
	}
	actual := canonicalGoldenJSON(t, recorder.Body.Bytes())
	path := filepath.Join("testdata", "golden", "graph.json")
	if *updateGolden {
		if err := os.MkdirAll(filepath.Dir(path), 0o750); err != nil {
			t.Fatalf("MkdirAll() error = %v", err)
		}
